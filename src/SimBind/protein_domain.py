...
def parse_domtblout(domtblout_file, fasta_file=None, add_15 = True):
    """
    ...
    if fasta_file:
        for p_id, seq in protein_seqs.items():
            if p_id in results:
                pass
            else:
                results[p_id] = [{'domain_name':"None", 'ali_from': 1, 'ali_to': (len(seq)), 'cEvalue':1.0, 'domain_seq':seq}]
...
def main():
    ...
    hmm_dir = os.path.join(THIS_DIR, "../../models/SimBind/HMM")
    ...
    domtblout_path = os.path.join(output_dir, run_id, f"hmmscan_results_{nuc_type}.tbl")
    fasta_out_path = os.path.join(output_dir, run_id, f"{nuc_type}_domains_plus_15.fasta") 
    ...
    if nuc_type == "RNA":
        hmm_db = os.path.join(hmm_dir, "combined_rna_binding.hmm")
    elif nuc_type == "DNA":
        hmm_db = os.path.join(hmm_dir, "combined_dna_binding.hmm")
    else:
